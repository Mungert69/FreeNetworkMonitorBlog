...
for (const post of JSON.parse(data)) {
   ...
   const frontmatter = `---\n${Object.keys(post.frontmatter).map((key) => {
     // Handle categories, questions, and answers as arrays
     if (['categories', 'questions', 'answers'].includes(key) && Array.isArray(post.frontmatter[key])) {
       return `${key}: [${post.frontmatter[key].map((item) => `"${String(item).replace(/"/g, '\\"')}"`).join(', ')}]`;
     } else {
